                                ...
                                $.ajax({
                                    ...
                                    success: function(data){
                                        ...
                                        if (data.stateCode === 200) {
                                            ...
                                            console
                                                .log("买票时完善个人信息成功，跳转到购票页");
											/*可能有缓存，清除一下。实在不行，可以返回选择购票页*/
                                            window.location.href = 'booking.html';
                                        } else {
                                            // 注册失败,错误信息在data.msg里面
                                            console
                                                .log("完善个人信息的页面的json信息提交失败,alert输出data.msg");
                                            alert("错误信息："
                                                + data.msg);
...
function booking(data) {
	if (data){
		window.localStorage.setItem("curTicket", JSON.stringify(data));
		window.location.href = "booking.html";
	}else {
		alert('无法获取该车次信息，情重试。')
	}
...
function pay() {
	...
	var storage = window.localStorage;
	var username = storage["username"];
	var ticketItem = storage.getItem("curTicket");
	...
	$.ajax({
		...
		success : function(data) {
			if (data.stateCode == "200") {
				...
				$("#carNum").val(ticketItem.carNum);
				$("#orginLocation").val(ticketItem.orginLocation);
				$("#destinationLocation").val(ticketItem.destinationLocation);
				$("#startTime").val(ticketItem.startTime);
				$("#reachTime").val(ticketItem.reachTime);
				$("#ticketPrice").val(ticketItem.ticketPrice);
				//storage.removeItem('curTicket');// 清除留存的信息
			} else{
				// 查询失败,错误信息在data.msg里面（未填写个人信息，请完善个人信息）
				alert("错误信息：" + data.msg)
				console.log("跳转到填写个人信息页面")
